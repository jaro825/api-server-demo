...
var (
	...
	ErrCacheTTLNotFound     = errors.New("CACHE_TTL not found in configuration file and env var")
	ErrAPIPortNotFound      = errors.New("API_PORT not found in configuration file and env var")
...
func LoadConfig(path string) (Config, error) { //nolint: cyclop
	...
	if err := v.ReadInConfig(); err != nil { // nolint: nestif
		...
		if errors.As(err, &configFileNotFoundError) { // nolint: nestif
			...
			if err = v.BindEnv("CACHE_TTL"); err != nil {
				return Config{}, ErrCacheTTLNotFound
			}
